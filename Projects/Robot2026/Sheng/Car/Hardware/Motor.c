...
void Motor_SetPWM(Motor_Typedef *Motor , int PWM)
{
	...
	if (PWM >= 0)
	{
		My_GPIO_WritePin(Motor->Motor_GPIO_IN2 , 0 ) ;	// 低电平,0
		MyPWM_SetCompare(Motor->Motor_PWM , PWM) ;			// 变大
	
	}
	else
	{
		My_GPIO_WritePin(Motor->Motor_GPIO_IN2 , 1 ) ;															// 高电平,1
		MyPWM_SetCompare(Motor->Motor_PWM , Motor->Motor_PWM->PWM_MAX + PWM) ;			// 变小
	}
}
...
void Motor_Speed_Update(Motor_Typedef *Motor , uint32_t Gap_Time_ms)
{
	...
	// 转速n = 总脉冲数/倍频(4)/单圈脉冲数(13)/减速比(30)/采样时间(Gap_Time_ms)
...
// 5. 得到当前电机走过的位移: 周长: 2 * 3.1415926 * 
#define Motor_ZhouChang (1)
void Motor_Pos_Update(Motor_Typedef *Motor)
{
	// 得到角度 = 圈数 * 360
	// 得到位移 = 圈数 * 2 * π * r = 圈数 * π * d =  圈数 * 周长
	float curr_Angle = (float)Motor->Motor_Encoder->total_cnt * Motor_ZhouChang / 
		(Motor->Motor_Encoder->time_Fre * Motor->Motor_Param->PPR * Motor->Motor_Param->ReductionRatio)   ;
