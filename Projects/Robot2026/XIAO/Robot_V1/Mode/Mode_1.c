...
void Mode_1_Setup(void)
{
   ...
   Motor_SetGoalSpeed(&Motor_A, 0) ;
   Motor_SetGoalSpeed(&Motor_B, 0) ;
}
...
{
   // OLED展示真实速度
   OLED_ClearArea(0, 20, 120, 10) ;
   OLED_Printf(0 , 20, OLED_6X8, "A:%.0f" ,  Motor_A.PID_s.realPoint_Now) ;
   OLED_Printf(60, 20, OLED_6X8, "B:%.0f" ,  Motor_B.PID_s.realPoint_Now) ;
   // Serial参数更改
   if (Serial_GetNewPackageFlag_ABC(&Serial1))
   {
       ...
       Motor_SetGoalSpeed(&Motor_A, Motor_A.PID_s.goalPoint) ;
       // OLED展示PID参数
       ...
       Motor_SetGoalSpeed(&Motor_B, Motor_B.PID_s.goalPoint) ;
       // OLED展示PID参数
