...
#define OLED_Addr 0x3c              // 7位从机地址
#define OLED_SCL_PIN 6              // PB6
#define OLED_SDA_PIN 5              // PB7
#define MASTER_FREQUENCY 100000     // 传输频率      
...
void OLED_WriteCommand(uint8_t Command)
{
    ...
    dat_buf[0] = 0x00;      // 控制字节：写命令
    dat_buf[1] = Command;
	
		HAL_I2C_Master_Transmit(&hi2c1 , OLED_Addr << 1 ,  dat_buf , 2 , 1000 ) ;
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    buf[0] = 0x40;                  // 控制字节：写数据
    memcpy(&buf[1], Data, Count);   // 拷贝数据区
	
		HAL_I2C_Master_Transmit(&hi2c1 , OLED_Addr << 1 ,  buf , sizeof(buf) , 1000 ) ;
}
